...
if ($result->num_rows > 0) {
    ...
    while ($rows = $result->fetch_assoc()) {
        ...
        $subjectslist = array();
        $subjectslist ['cartid'] = $rows['cart_id'];
        $subjectslist ['subname'] = $rows['subject_name'];
        $subprice = $rows['subject_price'];
        $subjectslist ['price'] = number_format((float)$subprice, 2, '.', '');
        $subjectslist ['cartqty'] = $rows['cart_qty'];
        $subjectslist ['subid'] = $rows['subject_id'];
        $price  = $rows['cart_qty'] * $subprice;
        $total_payable = $total_payable + $price;
        $subjectslist ['pricetotal'] = number_format((float)$price, 2, '.', ''); 
